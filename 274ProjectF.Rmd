...
```{r ARMA-modelID}
#Time Series for Model ID
  trip_H.t.d.1.D.168.s <- as.ts(trip_H.t.d.1.D.168[1:(length(trip_H.t.d.1.D.168)-168*2)])

#Auto.arima suggestion
  auto.arima(trip_H.t.d.1.D.168.s)
...
#Explore Parameter Range
  ...
  for(i in 1:12){
    for(j in 1:12){
      #Model
      ARMA_can <- arima(trip_H.t.d.1.D.168.s, order = c(i,0,j))
      ...
      for(m in 0:4){
        for(n in 0:4){
          ...
          #ARIMA
            ...
            s <- c(6)
          #Model
            SARIMA_can <- sarima.can(trip_H.t.d.1.D.168.s, ARIMA_v, SARIMA_V, Seas[k])
  ...
  for(i in 0:2){
    for(j in 0:2){
      for(k in 1:length(Seas)){
        ...
        #Save output   
          write.csv(SARIMA_summ, "SARIMA_summ.final.csv")
        }
      }
    }
  }
